...
module.exports =
    ...
    registerHbs: (handlebars)->
        ...
        '''
        {{#ifPage}}<a href="{{meta.url}}" class="thumb_image page_link_thumb_a"  style="background-image: url('{{meta.media}}')"><span class='title'>{{defaultNoUnderscores meta.title meta.name}}</span><span class='sum'>{{{meta.sum}}}</span></a>{{/ifPage}}
        '''
